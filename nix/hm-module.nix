...
in
{
  ...
  config =
    let
      ...
      shellConfigSource = pkgs.writeText "caelestia-shell.json" (mkConfig cfg);
      cliConfigSource = pkgs.writeText "caelestia-cli.json" (mkConfig cfg.cli);
    ...
    lib.mkIf cfg.enable {
      systemd.user.services.caelestia = lib.mkIf cfg.systemd.enable {
        Unit = {
          ...
          X-Restart-Triggers = lib.mkIf (shouldGenerate cfg) [
            "${shellConfigSource}"
          ];
        };
      ...
      home.activation.writeCaelestiaConfig = lib.hm.dag.entryAfter [ "writeBoundary" ] ''
        ...
        ${lib.optionalString (shouldGenerate cfg) ''
          install_mutable_config "${shellConfigSource}" "${config.xdg.configHome}/caelestia/shell.json"
        ''}

        ${lib.optionalString (shouldGenerate cfg.cli) ''
          install_mutable_config "${cliConfigSource}" "${config.xdg.configHome}/caelestia/cli.json"
        ''}
      '';
